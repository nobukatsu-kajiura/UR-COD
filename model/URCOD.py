...
class Generator(nn.Module):
    def __init__(self, channel, latent_dim):
        ...
        self.xy_encoder = Encoder_xy(10, channel, latent_dim)
        self.x_encoder = Encoder_x(9, channel, latent_dim)
        self.tanh = nn.Tanh()
        self.cenet = CENet()
    ...
    def forward(self, x, pseudomask, y=None, training=True):
        if training:
            ...
            pseudo = torch.cat((pseudomask, pseudoedge), 1)

            self.posterior, muxy, logvarxy = self.xy_encoder(torch.cat((x, pseudo, y), 1))
            self.prior, mux, logvarx = self.x_encoder(torch.cat((x, pseudo), 1))
            ...
            return self.prob_pred_post, self.prob_pred_prior, lattent_loss, self.pseudo_pred_post, self.pseudo_pred_prior, pseudoedge
            
        else:
            pseudoedge = self.cenet(x)
            pseudo = torch.cat((pseudomask, pseudoedge), 1)
            _, mux, logvarx = self.x_encoder(torch.cat((x, pseudo), 1))
            z_noise = self.reparametrize(mux, logvarx)
            self.prob_pred, _  = self.sal_encoder(x, pseudo, z_noise)
